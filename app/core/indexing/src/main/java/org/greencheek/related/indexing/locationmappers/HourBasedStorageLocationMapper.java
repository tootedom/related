...
/**
 * Creates an index name based on the current hour : [indexnameprefix]-yyyy-MM-dd_HH
 */
public class HourBasedStorageLocationMapper implements RelatedItemStorageLocationMapper {
    ...
    public String getLocationName(RelatedItem product) {
        ...
        String date;
        if(dateStr==null) {
            date = currentDayFormatter.getCurrentDayAndHour();
        } else {
            date = currentDayFormatter.parseToDateAndHour(dateStr);
        }
